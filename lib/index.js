...
function buildSummary(data){
  ...
  try{
    data.forEach(feature=>{
      feature.elements.forEach((scenario,jj)=>{
        ...
        scenario.steps.forEach(step=>{
          summary.step.total++
          slow.push({
            duration:parseInt(step.result.duration/1000000000),
            desc:step.name
          })
...
console.log("Going to build report")
  let os=[]
  Object.keys(summary.issue).map(x=>{
    let xd=summary.issue[x]
    if(!Object.keys(xd).length){
      return ""
    }
    Object.keys(xd).forEach(a=>{
      xd[a].id=a
      xd[a].issue=x
      os.push(xd[a])
    })
  })
  os.sort((a,b)=>b.total-a.total)
  let issueHtml=os.map(o=>{
    return `<tr onClick='toggleRow(this)'>
      <td>${o.total}</td>
      <td>${o.id}</td>
      <td>${o.errDesc||""}</td>
      <td>${o.issue=="unknow"?"unknown":o.issue}</td>
      <td>${o.scope||""}</td>
      <td>${o.status||""}</td>
      <td>${o.assignTo||""}</td>
      <td>${o.url?"<a href='"+o.url+"' style='white-space:nowrap;' target='_blank'>"+(o.key||"- Go -")+"</a>":""}</td>
      <td style='${o.ide?"":"display:none;"}'><a href='${o.ide}' target='bz-master' style='white-space:nowrap;text-decoration:none;'>${o.desc?"✐ Edit":"✚ Add"}</a></td>
      <td class="expanded-row-content hide-row">${o.desc||"[No issue mapped to error hash]"}</td>
    </tr>`
  ...
  let detailHtml=details.sort((a,b)=>a.start>b.start?1:-1).map(x=>{
  ...
  try{
    ...
    fs.writeFileSync(`bz-report-` + formattedDate + `.html`, html);    
  }catch(ex){
    console.log(ex.message)
  }
}
